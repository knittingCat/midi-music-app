...
- Runs of three or more evenly spaced notes that fit eighth- or quarter-note triplets better than any straight value are written as triplets. A run that doesn't divide into threes is completed with triplet rests when a rest follows it (four notes then a pause become `♪♪♪ | ♪𝄾𝄾`); otherwise the leftover notes take a straight value. A triplet group never crosses a barline.
- A pause longer than 3 seconds ends the phrase: the bar is completed with rests and the next chord starts a fresh bar, instead of the pause being written out. Held chords longer than a whole note are written tied across bars.
...
- Rhythm comes from the time between chord onsets at the current tempo. A run of notes whose spacing stays close to the run's average shares one value rather than flickering between eighths and sixteenths (so a slightly swung trill is still written evenly); a short last note before a rest takes the run's value too. Each value is chosen from the two plausible readings either side of what was played, whichever keeps the written length closest to the played length — including a small carry-over of timing error from the preceding notes, so the score doesn't slowly drift away from the barlines. Rests absorb most of that error; it also fades quickly, so a rushed phrase doesn't distort the next one. A dotted eighth is only written when it's paired with a sixteenth. A gap of at least an eighth note between releasing a key and pressing the next one becomes a rest; shorter gaps are treated as legato.
